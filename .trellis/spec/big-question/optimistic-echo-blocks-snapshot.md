...
这一句，中间没有助手回复、没有 bash 工具卡；输入框锁着「消息将排队」。磁盘 jsonl
与 `settings.json` 里的工具输出都在往前推进——worker 是好的，坏的只是 renderer 投影。

历史症状还包括底下「距上次返回」被越界 upsert 续命。`lastOutputAt` 现已改为只认可见
生成（非空 text/thinking、toolResult、非空 tool-output），越界丢正文不再刷新心跳。
...
3. worker 推来的 `message-upsert` 带原 index（一百多）。reducer 里
   `index > 权威区长度` 时只推 `seq` 并丢正文（这本是为了防稀疏空洞崩溃），且
   **没有任何人再去要快照**。正文进不了时间线。bash stdout 走 `tool-output`
   另一条路进 `toolOutputs`，但没有工具卡可绑，同样不可见。
   越界路径**不得**写 `lastOutputAt`：否则 stall watchdog 把脱节心跳当成输出，
   「无输出则停止」永远不触发。
...
- reducer 的 `snapshot` 分支不再整段替换：快照里已有同文本 user 消息的乐观回显
  ...
  那句抹掉，又是另一种「消息凭空消失」。
- `lastOutputAt` 只在可见生成输出时刷新；越界 upsert 只推 `seq`。
...
- 时间线为空但 composer 仍锁 running，先 dump `conversation.messages` 看是否只剩
  `optimistic: true` 的条目，再看 jsonl 尾部时间戳——两者错位即此坑。
- 「无输出则停止」不触发但界面已无 token：dump `lastOutputAt` 是否被非可见 upsert
  续命（越界、空 assistant、仅 toolCall）。心跳必须跟时间线可见输出对齐。
